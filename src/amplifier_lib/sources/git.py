...
class GitSourceHandler:
    ...
    def _build_git_url(self, parsed: ParsedURI) -> str:
        ...
        Applies shadow environment URL rewriting when ``AMPLIFIER_GIT_HOST``
        is set, redirecting ``github.com`` URLs to a private mirror for
        air-gapped or shadow-environment testing.
        """
        scheme = parsed.scheme.replace("git+", "")
        url = f"{scheme}://{parsed.host}{parsed.path}"

        shadow_host = os.environ.get("AMPLIFIER_GIT_HOST")
        if shadow_host and "github.com" in url:
            clean = url.rstrip("/")
            ...
            if len(parts) == 2:
                ...
                if len(path_parts) >= 2:
                    repo = path_parts[1]
                    rewritten = f"{shadow_host.rstrip('/')}/amplifier/{repo}"
                    logger.debug("Shadow URL rewrite: %s -> %s", url, rewritten)
                    return rewritten
    ...
    def _get_cache_metadata(self, cache_path: Path) -> dict:  # type: ignore[type-arg]
    ...
    def _save_cache_metadata(self, cache_path: Path, metadata: dict) -> None:  # type: ignore[type-arg]
        """Save cache metadata."""
        meta_path = cache_path / CACHE_METADATA_FILE
