"""Module activation for amplifier-lib.
...
class ModuleActivator:
    ...
    def __init__(
        ...
        self._resolver = self._build_source_resolver(base_path)
    ...
    def _build_source_resolver(self, base_path: Path | None) -> Any:
    ...
    async def activate(
        ...
        if self.install_deps:
            await self._install_dependencies(module_path, module_name, progress_callback)
    ...
    async def activate_bundle_package(
        ...
        # Some bundles place their shared package under lib/ instead of src/,
        # so we need to cover both conventions.
    ...
    def _build_git_dep_overrides(pyproject_path: Path) -> list[str]:
        ...
        for dep in deps:
            ...
            try:
                ...
                logger.debug(
                    f"Overriding git dependency '{pkg_name}' with installed version {version}"
                )
